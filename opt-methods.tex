...
random environment. % Don't understand -- clarify - JJ
 ...
 parameters requires a system of equations that, in some cases, has
 no explicit solution; thus, the solution must be estimated. In this paper, we propose several ways to solve this problem using some optimization methods to find those solutions: an Iterative
 ...
 Evolutionary Algorithm. Four data sets following a
 Gompertz-lognormal diffusion process have been tested with different noise
...
The methods are applied to these data sets in order to
 ...
 all four data sets, even on instances of the problem with higher noise level. 
 %noise is
 %first mentioned here. Why is it
 %important? - JJ
 Moreover, some analytical methods as Newton-Raphson or the Iterative Method do
 not always find the solution for the problem because their scores depend on the
 starting point for initial solution or because the noise level hinders the
 resolution of the problem. In these cases, the bio-inspired
 algorithms remain a suitable and reliable approach.
...
growth-patterns and making forecasts of data.% of what? - JJ
Although there exists a lot of
different growth-patterns, and many different ways of modeling them,
among the most widely
studied and useful models are diffusion processes \cite{rogerswilliams1} % cita - JJ
