...
\title{Using evolutionary algorithms to optimize the estimation of a diffusion process\titlenote{(Produces the permission block, and
...
% 1st. author
\alignauthor
%Nuria Rico\titlenote{Dr.~Nuria Rico Castro.}\\
...
process requires to solve a system of equations that, in some cases, has no
...
Many probabilistic models are frequently used for natural
growth-pattern modeling and making forecast. Although there exists a
lot of different growth-patterns, and many models to use, among the
widely studied and useful models are diffusion processes. They are
frequently employed in biological growth modeling. \cite{lognormal}. 


%%%Nuevo

A diffusion process is an usual mathematical formulation of a model for population growth in a randomly
varying environment \cite{lognormal}. These models describe how a
population adopts a new innovation, technology or behaviour. With
these kind of models, we have the transity density function (t.d.f.)
% transity? Is that correct? - JJ
which express the chance of observing 
any size for the population at any time when the initial size is
specified. With the t.d.f we also are able to determine the time at
that population first reaches a fixed size. Moreover, we can forecast
future values of the population size as well as confidence intervals
for the predictions. 

In this paper we focus our attention on the Gompertz-lognormal
diffusion process \cite{proceedingBol}. It can be seen as a mixture
between a Gompertz-type process \cite{Gut07b} and a lognormal process
\cite{GutCy}. The first one models precesses with trends showing
enclosed growth patterns, as biological growth with finite space or
resources. The second one is useful for modelling unbounded growth
with an exponential tendency. 

The mixture of them, the Gompertz-lognormal diffusion process,
combines both trends; a first part Gompertz-like growth and a second
exponential part. This process is defined through the infinitesimal
moments 
%{\scriptsize
\[
\begin{array}{l}
...
%}
%Maribel: Deber{\'\i}amos poner qu{\'e} es m, que es e, que es c y por
%supuesto mencionar que t es el tiempo y que x es la variable
%independiente no? 

Taking $c=0$ the process is Gompertz-type, and for a known fixed value
of $\beta$ the process is an homogeneous lognormal diffusion
process. In other cases, the process has a shape with two inflection
points. This makes the process useful for modelling situations with
observed paths along time that turn from increasing to decreasing or
...
\bibitem{Gut99} Guti{\'e}rrez, R., Rom{\'a}n, P., Torres, F.:
